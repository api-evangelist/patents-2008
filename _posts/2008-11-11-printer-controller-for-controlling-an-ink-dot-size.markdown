...
---
This application is a Continuation application of U.S. Ser. No. 10 636 227 filed Aug. 8 2003 which is a Divisional Application of U.S. Ser. No. 09 436 747 filed on Nov. 9 1999 now Issued U.S. Pat. No. 6 687 022 all of which are herein incorporated by reference.
...
In one broad form the in comprise a method of controlling the dot size of the ink discharged by a nozzle of a printhead comprising the steps of

changing the duration of the firing pulse to compensate for the viscosity of the ink due to temperature so as to produce a uniform ink dot size.

In another form the invention comprises An apparatus for controlling the dot size of the ink discharged by a nozzle of a printhead said print head having a plurality of ink discharge nozzles and pulse means to generate a pulse in each nozzle discharge the ink comprising

a temperature sensor to sense the temperature of the ink in the nozzles pulse generating mechanism to generate a pulse to each nozzle of variable durations controller communicating with said temperature sensor and said pulse generator whereby the controller adjusts the pulse duration for each nozzle in response to the temperature of each nozzle so as to provide a uniform dot size during a print cycle.
