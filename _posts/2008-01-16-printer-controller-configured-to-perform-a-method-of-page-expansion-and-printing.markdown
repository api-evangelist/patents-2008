...
title: Printer controller configured to perform a method of page expansion and printing
...
url: http://patft.uspto.gov/netacgi/nph-Parser?Sect1=PTO2&Sect2=HITOFF&p=1&u=%2Fnetahtml%2FPTO%2Fsearch-adv.htm&r=1&f=G&l=50&d=PALL&S1=07808677&OS=07808677&RS=07808677
...
---
The present application is a continuation of U.S. application Ser. No. 11 764 729 filed on Jun. 18 2007 which is a continuation of U.S. application Ser. No. 11 298 634 filed on Dec. 12 2005 now issued as U.S. Pat. No. 7 245 399 which is a continuation of U.S. application Ser. No. 10 659 017 filed on Sep. 11 2003 now issued as U.S. Pat. No. 7 061 644 which is a continuation of U.S. application Ser. No. 09 436 909 filed on Nov. 9 1999 now issued as U.S. Pat. No. 6 665 094 the entire contents of which are herein incorporated by reference.

This invention concerns a pagewidth printhead controller circuit for a high performance printer such as a printer using a pagewidth color inkjet printhead.

In one aspect the invention is an integrated circuit fabricated on a single substrate. It includes a first interface to a page width inkjet printhead and a general purpose processor that is connected to the first interface. The processor runs software that controls the first interface.

The first interface may control the printing of pages in streaming mode or in single page mode. Single page mode may be used when the size of received pages exceeds a memory threshold that may be on the order of 3 MB.

The invention may also include a Memjet printhead connected to the first interface. The first interface may load the Memjet printhead with dots to be printed and may also control a printing process.

The first interface may also include a line loader format unit that loads the dots to be printed into local buffer storage units and formats the dots into an order required for the Memjet printhead. A Memjet second interface that is part of the first interface may also transfer data to the Memjet printhead and control a firing sequence of printhead nozzles during a print process.

The line loader format unit may use a double buffering scheme for preparing the dots for printing where one buffer unit is read from while another buffer unit is written to. The local buffer storage units may also be divided into multiple sub buffers where each sub buffer is assigned a single color.
