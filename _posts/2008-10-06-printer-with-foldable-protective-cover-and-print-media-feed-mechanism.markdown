...
---
The present application is a Continuation Application of U.S. application Ser. No. 11 124 231 filed May 9 2005 which is a Continuation Application of U.S. application Ser. No. 10 659 028 filed Sep. 11 2003 now issued as U.S. Pat. No. 7 218 415 which is a Continuation Application of U.S. application Ser. No. 09 436 909 filed on Nov. 9 1999 now issued as U.S. Pat. No. 6 665 094 the entire contents of which are herein incorporated by reference.

This invention concerns an integrated circuit for a high performance printer such as a printer using a page width color inkjet printhead.

According to one aspect of the invention there is provided a printer controller for an inkjet printer the controller comprising 

connections for linking the microprocessor to an external USB module a printhead a paper transport mechanism and a print media sensor 

In another aspect the invention there is provided an integrated circuit fabricated on a single substrate. The integrated circuit includes a data bus and purpose specific functional units connected to the data bus. The functional units include a JPEG decoder a compressed bi level expander a halftoner and a printhead interface.

A general purpose processor may also be connected to the data bus for controlling the functional units. The processor may be connected to run software that coordinates the functional units to receive expand and print pages.

The processor may print pages in streaming mode or print pages in single page mode when the size of received pages exceeds a memory threshold that may be on the order of 3 MB.

The JPEG decoder and the compressed bi level expander may decompress in parallel both a bi level layer and a contone layer.

The halftoner may halftone contone color data to a bi level color layer and then may composite a bi level black layer over the bi level color layer.

The halftoner may halftone contone CMYK data to a bi level CMYK layer and then may composite a bi level black layer over the bi level CMYK layer.

The processor may transfer bi level CMYK data to the printhead interface at a constant required rate. The processor may also expand pages in real time during printing.

The invention may also include a multi channel DMA controller connected to the data bus. Each of the functional units may then include one or more on chip input and or output FIFOs and each FIFO may be allocated a separate channel in the multi channel DMA controller. The DMA controller may interrupt the processor when a data transfer is complete.

The compressed bi level expander unit may also include a bitstream decoder a state machine connected to the bitstream decoder edge calculation logic connected to the state machine two runlength decoders connected to the edge calculation logic and a runlength re encoder connected to the edge calculation logic. The state machine may control the edge calculation logic in response to codes supplied by the bitstream decoder.
...
The input contone CMYK FIFO is a full 8 KB line buffer. The line is used contone scalefactor times to effect vertical up scaling via line replication. FIFO write address wrapping is disabled until the start of the last use of the line. An alternative is to read the line from main memory contone scale factor times increasing memory traffic by 65 MB s but avoiding the need for the on chip 8 KB line buffer.
...
The printing uses a double buffering scheme for preparing and accessing the dot bit information. While one line is being loaded into the first buffer the pre loaded line in S the second buffer is being read in Memjet dot order. Once the entire line has been transferred from the second buffer to the printhead via the Memjet interface the reading and writing processes swap buffers. The first buffer is now read and the second buffer is loaded up with the new line of data. This is repeated throughout the printing process as can be seen in the conceptual overview of
