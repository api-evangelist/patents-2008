...
title: Method of page expansion and printing with a pagewidth printer having low-speed and high-speed firing modes
...
url: http://patft.uspto.gov/netacgi/nph-Parser?Sect1=PTO2&Sect2=HITOFF&p=1&u=%2Fnetahtml%2FPTO%2Fsearch-adv.htm&r=1&f=G&l=50&d=PALL&S1=07817306&OS=07817306&RS=07817306
...
---
The present application is a continuation application of U.S. application Ser. No. 11 737 142 filed on Apr. 19 2007 which is a continuation application of U.S. application Ser. No. 10 636 288 filed on Aug. 8 2003 now issued as U.S. Pat. No. 7 277 205 which is a divisional of U.S. application Ser. No. 09 436 747 filed on Nov. 9 1999 now issued as U.S. Pat. No. 6 687 022 all of which are herein incorporated by reference.
...
assigning for each dither cell location a fixed set of 3 thresholds defining 4 intensity intervals within which said dither cell location is defined to an alternative not set and set.
...
Runlength decoder decodes the output runlength to a line buffer large enough to hold an 8 1600 dpi line 1600 bytes . The runlength passed to this output runlength decoder is multiplied by the scale factor so this decoder produces 1600 dpi lines. The line is output scalefactor times through the output pixel FIFO . This achieves the required vertical scaling by simple line replication. The EEU could be designed with edge smoothing integrated into its image scaling. A simple smoothing scheme based on template matching can be very effective 10 . This would require a multi line buffer between the low resolution runlength decoder and the smooth scaling unit but would eliminate the high resolution runlength decoder.
